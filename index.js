...
app.get("/nominatim", function (request, response) {
  fetch(
    `https://nominatim.openstreetmap.org/reverse?lat=${request.query.lat}&lon=${request.query.lon}&format=json`,
    {
      headers: {
...
app.get("/refresh", function (request, response) {
  fetch("https://login.herecomesthebus.com/Map.aspx/RefreshMap", {
    ...
    body: JSON.stringify({
      legacyID: request.query.person,
      name: request.query.name,
      timeSpanId: request.query.time,
      wait: "false",
    }),
    method: "POST",
  })
...
app.get("/cookie", async function (req, res) {
  if (!req.query.user || !req.query.pass || !req.query.code)
    return res.json({
      success: false,
      message: "Please provide all credentals",
    });
