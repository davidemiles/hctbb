...
var UpdateControlPanel = function (...a) {
  document.querySelector("#panel").innerHTML = `${a[0]}<br>${a[1]}<br>${a[2]}<br>${a[3]}<br>${a[4]}<br${a[5]}<br>`
};
...
var SetBusPushPin = function (a,b) {
  ...
  document.querySelector("#map").innerHTML = `<div class="mapouter"><div class="gmap_canvas"><iframe width="600" height="500" id="gmap_canvas" src="https://maps.google.com/maps?q=${window.BusStopData}&t=k&z=17&ie=UTF8&iwloc=&output=embed" frameborder="0" scrolling="no" marginheight="0" marginwidth="0"></iframe><br><style>.mapouter{position:relative;text-align:right;height:500px;width:600px;}</style><style>.gmap_canvas {overflow:hidden;background:none!important;height:500px;width:600px;}</style></div></div>`

  if (navigator.geolocation) {
   
    fetch(`/nominatim?lat=${a}&lon=${b}`).then(res => res.json())
    .then(json => {
       navigator.geolocation.getCurrentPosition(function(position){
      const first = {latitude:position.coords.latitude,longitude:position.coords.longitude}
      const second = {latitude:a,longitude:b}
        document.querySelector("#buslocate").innerHTML = `The bus is at ${window.BusStopData} (${(haversineDistance(first,second)/1609.344).toFixed(3)} miles away)<br>${json.display_name}` 
    });
      })
  } else { 
